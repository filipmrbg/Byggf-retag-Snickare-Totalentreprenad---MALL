        ...
        <table role="presentation" style="max-width: 600px; width: 100%; background-color: #ffffff; border-radius: 8px; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06); overflow: hidden;">
          <tr>
            <td style="padding: 40px;">
              ...
              <p style="margin: 0 0 24px 0; font-size: 14px; color: #6b7280;">${escapeHtml(dateTime)}</p>
              ...
              <p style="margin: 0; font-size: 12px; color: #6b7280; line-height: 1.5;">
                Detta meddelande skickades automatiskt från kontaktformuläret på wshbygg.se.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const dateTime = formatSwedishDate();
    ...
    if (dbError) {
      return new Response(
        JSON.stringify({ error: "Kunde inte spara förfrågan. Försök igen." }),
    ...
    return new Response(
      JSON.stringify({ error: "Ett oväntat fel uppstod. Försök igen." }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  }
});
